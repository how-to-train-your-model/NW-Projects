...
@app.route("/home", methods=["GET", "POST"])
def home():
    pageTitle = "Statefarm Application Search"
...
def extractIntent(UserInput):
    ...
    if UserInput == "who is the largest competition":
        SQL_Results = runCypher("""
            
        SELECT name, totalemp FROM cypher('sf_graph', 
                                          $$ MATCH (v:Company)
    ...
    if UserInput == "who are the new entrant":
        SQL_Results = runCypher("""
            
        SELECT name, totalemp, founded FROM cypher('sf_graph', 
                                                   $$ MATCH (v:Company)
                                                   WHERE v.attributes.founded > 2010
                                                   RETURN DISTINCT v.name,
                                                   v.attributes.numberEmployees,
                                                   v.attributes.founded
                                                   $$) as (name agtype, totalemp agtype,  founded agtype)
        ORDER BY totalemp desc
        limit 10;
        """)
        SQL_Results['humanReadableQueryVerify'] = "Testing PostgreSQL and Cypher Query Results"
    ...
    if UserInput == "who is the oldest competition":
        SQL_Results = runCypher("""
            
        SELECT name, totalemp, founded FROM cypher('sf_graph', 
                                                   $$ MATCH (v:Company)
                                                   WHERE v.attributes.founded <= 1922
                                                   RETURN DISTINCT v.name,
                                                   v.attributes.numberEmployees,
                                                   v.attributes.founded
                                                   $$) as (name agtype, totalemp agtype,  founded agtype)
...
def runTextSearch(input):
    ...
    SELECT id, ts_rank_cd(textsearchable_index_col, query) AS rank
        FROM Statefarm_rel.Statefarm_docs, to_tsquery('{input}') query
...
def runTextMatchSummary(ID_list, SearchParamater):
   ...
   from (select *, to_tsquery('english', '{SearchParamater}') query 
         from Statefarm_rel.Statefarm_docs where id in({idlst})
         ) a
  ;
    """
    print("----------")
